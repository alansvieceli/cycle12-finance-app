...
Updated after migrating the primary test runner to Jest/Expo, adding a small React Native Testing Library render test, covering the local storage adapter, adding rolling-window propagation tests, and covering installment month selection:
...
It also includes a small component render test to validate the React Native Testing Library setup, focused storage adapter tests for AsyncStorage persistence behavior, rolling-window propagation tests, and installment month selection tests.
...
- `src/lib/installmentMonths.ts`: mostly covered, with one defensive invalid-count branch remaining.
